<script>
    import { forgottenAlbums } from "~/logic/album.js";
    ...
    $: query = createQuery({
        queryKey: ["forgottenAlbums"],
        queryFn: async () => {
            let result = await forgottenAlbums({ limit: 1000 });

            if (result.error) {
                errorHandler("getting forgotten albums", result.error);
                return [];
            }
