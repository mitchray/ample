<script>
    ...
    const { _virtualList, dataDisplay, dataFinal, offsetY, listerObject, listerScroller, listerContainer, rowHeight, listerHeight, pseudoHeight } = getContext(contextKey);
    ...
    function update() {
        if (ready) {
            startNode = Math.floor($listerScroller.scrollTop / $rowHeight) - nodePadding;
            startNode = Math.max(0, startNode);

            visibleNodesCount = Math.ceil(parseInt($listerHeight) / $rowHeight) + (2 * nodePadding);
            visibleNodesCount = Math.min($dataDisplay.length - startNode, visibleNodesCount);

            $offsetY = startNode * $rowHeight;

            $dataFinal = $dataDisplay.slice(startNode, startNode + visibleNodesCount).map((d, i) => {
                return d;
            })
        }
    }
    ...
    async function handleScroll() {
        $listerContainer.scrollIntoView({block: "start", inline: "nearest", behavior: "smooth"});
